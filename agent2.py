...
def fetch_trackier_performance():
    ...
    try:
        ...
        while True:
            ...
            # DEBUG: print raw fields once so we can verify correct keys in Render logs
            if not debug_printed:
                print("   🔍 DEBUG raw record keys:", json.dumps(records[0], indent=2)[:1000])
                debug_printed = True
            ...
            for rec in records:
                ...
                # Use APPROVED-only figures, falling back through likely field names
                approved_revenue = rec.get("approvedRevenue",
                                    rec.get("approved_revenue",
                                    rec.get("revenue", 0)))
                approved_payout = rec.get("approvedPayout",
                                  rec.get("approved_payout",
                                  rec.get("payout", 0)))
                approved_conversions = rec.get("approvedConversions",
                                       rec.get("approved_conversions", 0))
                ...
                supabase.table("trackier_conversions").upsert({
                    ...
                    "publisher_id":  str(rec.get("publisher_id", "")),
                    "goal_value":    rec.get("campaign_name", ""),
                    "revenue":       float(approved_revenue or 0),
                    "payout":        float(approved_payout or 0),
                    "conversions":   int(approved_conversions or 0),
                    "clicks":        int(rec.get("clicks") or 0),
                    "status":        "approved",
                    "created_at":    today,
                    "synced_at":     now_ist().isoformat()
